  ...
  return (
    <div className="flex h-10 items-center overflow-hidden rounded-lg border border-white/10 bg-white/5">
      ...
      <input
        ...
        className="h-full flex-1 bg-transparent px-3 text-center text-[13px] text-white placeholder:text-white/30 outline-none"
      />
      <button
        type="button"
        onClick={() => onChange(Math.max(min, (Number(value) || 0) - step))}
...
export default function CreateEventModal({ open, onClose, onCreate }) {
  ...
  const canSubmit = useMemo(() => {
    return (
      name.trim().length > 0 && country && city && date && startTime && endTime
    );
  }, [name, country, city, date, startTime, endTime]);

  useEffect(() => {
    if (!open) return;
    // reset each open (matches the “empty create modal” screenshot)
    startTransition(() => {
      setName("");
      setCountry("");
      setCity("");
            ...
            <Field label="Cover Photo (optional)">
              ...
              {file && (
                <div className="mt-3 flex items-center justify-between rounded-lg border border-white/10 bg-white/5 p-3">
                  <div className="text-[12px] text-white/70">{file.name}</div>
                  <button
                    type="button"
